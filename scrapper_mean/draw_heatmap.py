...
def draw_fc_heatmap(drugs_to_draw, scale):
    ...
    fc_data = pd.read_csv('data/data.fc.tsv', sep='\t').set_index('drug').loc[drugs_to_draw].drop('BY4741', axis=1)
    problems_data = pd.read_csv('data/data.fc-prblms.tsv', sep='\t').set_index('drug').loc[drugs_to_draw].drop('BY4741', axis=1)
    # plot heatmap
