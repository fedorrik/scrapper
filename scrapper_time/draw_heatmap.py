...
def draw_fc_heatmap(drugs_to_draw, scale):
    ...
    fc_data = pd.read_csv('data/data.fc.tsv', sep='\t').set_index('drug').drop('BY4741', axis=1).loc[drugs_to_draw]
    problems_data = pd.read_csv('data/data.fc-prblms.tsv', sep='\t').set_index('drug').drop('BY4741', axis=1).loc[drugs_to_draw]
    # plot heatmap
    ...
    plt.ylabel('')
    plt.xlabel('')
    plt.savefig('heatmaps/fc-heatmap.height{}.scale{}.{}.pdf'.format(n_drugs, scale, datestamp), facecolor='white', bbox_inches='tight', pad_inches=0.1)
...
def draw_z_heatmap(drugs_to_draw, scale):
    ...
    n_drugs = len(drugs_to_draw)
    # read data.tsv, take selected drugs
    data = pd.read_csv('data/data.z.tsv', sep='\t').set_index('drug').loc[drugs_to_draw].drop('BY4741', axis=1)
    # plot heatmap
    pic_height = n_drugs*0.29
    datestamp = str(date.today()).replace('-', '')
    cbar_dict = {'location': 'top', 'pad': 0, 'aspect': 30, 'shrink': 0.5, 'label': 'z-score'}
    plt.figure(figsize=(16, pic_height))
    heatmap = sns.heatmap(data.fillna(0), mask=data.isnull(), vmin=-scale, vmax=scale, center=0, cmap='coolwarm', linewidths=0.5, linecolor='grey', cbar_kws=cbar_dict)
    heatmap.set_facecolor('xkcd:grey')
    heatmap.set_xticklabels(heatmap.get_xticklabels(), rotation=60)
    plt.ylabel('')
    plt.xlabel('')
    plt.savefig('heatmaps/z-heatmap.height{}.scale{}.{}.pdf'.format(n_drugs, scale, datestamp), facecolor='white', bbox_inches='tight', pad_inches=0.1)

def draw_dead_heatmap(drugs_to_draw, scale):
    n_drugs = len(drugs_to_draw)
    # read data.tsv, take selected drugs
    data = pd.read_csv('data/data.dead.tsv', sep='\t').set_index('drug').loc[drugs_to_draw].drop('BY4741', axis=1)
    # plot heatmap
    pic_height = n_drugs*0.29
    datestamp = str(date.today()).replace('-', '')
    ...
    plt.ylabel('')
    plt.xlabel('')
    plt.savefig('heatmaps/dead-heatmap.height{}.scale{}.{}.pdf'.format(n_drugs, scale, datestamp), facecolor='white', bbox_inches='tight', pad_inches=0.1)
